...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        stat = row
        val = formula(info, stat)
        tup = (stat[info["playerid"]],val)
        tup_list.append(tup)
    sorted_list = sorted(tup_list, key = lambda pair: pair[1], reverse=True)
    toptup = sorted_list[0:numplayers]
    return toptup
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for pair in top_ids_and_stats:
        playerid = pair[0]
        stat = '{:.3f}'.format(pair[1])
        if playerid in data:
            first = data[playerid][info["firstname"]]
            last =  data[playerid][info["lastname"]]
            line = "{} --- {} {}".format(stat, first, last)
            names.append(line)
    return names
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_list = top_player_ids(info, filter_by_year(stats, year, info["yearid"]),
                              formula, numplayers)
    top = lookup_player_names(info, top_list)
    return top
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    players = {}
    for row in statistics:
        if row[playerid] not in players:
            players[row[playerid]] = {}
            players[row[playerid]][playerid] = row[playerid]
            for field in fields:
                players[row[playerid]][field] = float(row[field])
        else:
            players[row[playerid]][field] += float(row[field])
